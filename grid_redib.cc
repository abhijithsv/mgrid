...
// Constructor
    ...
    {
        T = tensor;
        new_grid = new Grid(new_g->grid_dims,new_g->pgrid,new_g->grid_comm,new_g->grid_id, new_g->ngrids, new_g->all_grids);
        old_grid = T->g;
	...
	new_pgrid[new_grid->grid_dims]=1;
        ...
        new_grid->get_proc_addr(new_grid->rank, new_proc_addr);
...
//print_tile_addr(new_grid->grid_dims,new_grid->pgrid);}
	...
	for(int i=0;i<old_grid->grid_id;i++) start+=old_grid->all_grids[i]; end=start+old_grid->all_grids[old_grid->grid_id];
	MPI_Comm_rank(MPI_COMM_WORLD,&grank);
	if(grank>=start && grank<end){
	 repcount = get_replicated_dims(new_idx_map,repl_dims,grid_dims);
	...
	if(rank==RRANK && DEBUG_CHECK_POINT) cout << "Rank : "<<rank<<". Redistribute Checkpoint " <<(checkpoint++)<< endl; 
		...
		//int start=0,end,grank;
		start=0;
	for(int i=0;i<new_grid->grid_id;i++) start+=new_grid->all_grids[i]; end=start+new_grid->all_grids[new_grid->grid_id];
	MPI_Comm_rank(MPI_COMM_WORLD,&grank);
	if(grank>=start && grank<end){
	...
	if(rank==RRANK && DEBUG_CHECK_POINT) cout << "Rank : "<<rank<<". Redistribute Checkpoint " <<(checkpoint++)<<"T->num_actual_tiles"<<T->num_actual_tiles<< endl;
        ...
        T->tile_address = new int[T->dims * block_count+1];
	if(rank==RRANK && DEBUG_CHECK_POINT) cout << "Rank : "<<rank<<". Redistribute Checkpoint " <<(checkpoint++)<< endl;
...
void GridRedistribute::redistribute_send(int* &repl_dims, int repcount)
{
	...
	old_grid->get_proc_addr(old_grid->rank, proc_add);
	for(int i=0;i<repcount;i++)
	{
        ...
        for (int i=0; i < T->num_actual_tiles; i++)
        {
 			...
 			//cout<<"aaaaa"<<i<<endl;
                        if(num_blocks > 0)
                        {
                                ...
                                if(old_grid->get_global_rank(old_grid->rank) == new_grid->get_global_rank(new_proc_rank[i]))
                                {
					...
					/*if(T->rank == 3)
                                        ...
                                        MPI_Isend(blocks, num_blocks * T->block_size, MPI_DOUBLE, new_grid->get_global_rank(new_proc_rank[i]), 3, MPI_COMM_WORLD, &req1);
                                        MPI_Isend(block_addrs, num_blocks * dims, MPI_INT, new_grid->get_global_rank(new_proc_rank[i]), 4, MPI_COMM_WORLD, &req2);
...
// and post receives for them
	...
	{	
	    ...
	    new_grid->get_proc_addr(new_grid->rank, proc_add);
	    for(int i=0;i<repcount;i++)
	    {
		if(proc_add[repl_dims[i]]!=0) flag=false;
	    }
...
if(rank==RRANK && DEBUG_CHECK_POINT) cout << "Rank : "<<rank<<". Redistribute Recieve Checkpoint " <<(checkpoint++)<< endl; 
	...
	//cout<<"numtiles"<<num_tiles<<"  rank"<<T->rank<<endl;
      if(flag){         
		...
		if(new_grid->get_global_rank(new_grid->rank) == old_grid->get_global_rank(i))
		{	
		...
		{	
				//if(T->rank == RRANK) cout<<"I shouldn't be here"<<endl;
                                ...
                                MPI_Irecv(rd.blocks, num_blocks * T->block_size, MPI_DOUBLE, old_grid->get_global_rank(i), 3, MPI_COMM_WORLD, &req1);  
                                MPI_Irecv(rd.block_addrs, num_blocks * dims, MPI_INT, old_grid->get_global_rank(i), 4, MPI_COMM_WORLD, &req2); 
...
MPI_Comm_rank(MPI_COMM_WORLD,&rank);
	...
	int* new_proc_add = new int[grid_dims+1];
        new_grid->get_proc_addr(new_grid->rank, new_proc_add);
...
if(DDbug == 1) cout<<"Replication dimension is"<<rep_dim;
	...
	if(count>0){
	    if(new_grid->rank!=dim_group_ranks[0])
	    {
		delete[] T->tensor_tiles;
		delete[] T->tile_address;
