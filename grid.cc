...
// Constructor
Grid::Grid(int num_dims, int* phy_grid, MPI_Comm g_comm, int g_id, int num_grids, int* a_grids)
{	
	grid_id=g_id;
	...
	all_grids = new int[ngrids];
    memcpy(pgrid, phy_grid, grid_dims * sizeof(int));
	memcpy(all_grids,a_grids,ngrids*sizeof(int));
...
int start=0,end,grank;
	for(int i=0;i<g_id;i++) start+=a_grids[i]; end=start+a_grids[g_id];
...
int Grid::get_global_rank(int prank)
{	
	int grank=0;
	for(int i=0;i<grid_id;i++)
	{
		grank+=all_grids[i];
	}
	grank+=prank;
