...
// Gets receiver addresses for current contracting dimension and index
	...
	if(DEBUG_PRINT_TABLE && rank == 0)
	{
	    int t = MPI_Reduce(receiver_grid, receiver_grid_buffer, 16*16, MPI_INT, MPI_SUM, 0, grid->grid_comm);
	    cout<<endl;
	    for(int i =-1; i< 16; i++){
		cout<<i<<" :"<<setw(10);
		for(int j = 0; j < 16; j++){
	...
	//tile grid
	Grid* tgrid = new Grid(X->get_dims(), X->vgrid,X->g->grid_comm,X->g->grid_id, X->g->ngrids, X->g->all_grids);
...
// Receive data at instigator
	...
	if(contr_idx % pgrid[pindex] != my_address[pindex])
	{
		...
		MPI_Reduce(sender_grid, sender_grid_buffer, num_procs * num_procs, MPI_INT, MPI_SUM, 0, grid->grid_comm);
