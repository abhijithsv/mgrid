...
cost::cost(Tensor* &Ainit, Tensor* &Binit, Tensor* &Cinit, Grid* &Ginit)
    {
        ...
        dimG=G->grid_dims;
		nprocs=G->nprocs;
		cout<<"nprocs="<<nprocs<<"at "<<G->rank<<endl;
		found=false;
...
void cost::contract_2D_2D_4D(int* &idmapA, int* &idmapB, int* &idmapC, int* &ndimG, int* &npgrid)
{
	...
	{	
		...
		{
			if(!C->contr_dim_str[c].compare(A->contr_dim_str[a]))
			{
				idmapC[c]=idmapA[a];
			}
		}
	...
	{	
		...
		{
			if(!C->contr_dim_str[c].compare(B->contr_dim_str[b]))
			{
				idmapC[c]=idmapB[b];
			}
		}
