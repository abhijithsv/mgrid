...
namespace RRR{
	...
	if(grank<start || grank>=end){return;}
	grid = g;//new Grid(g->grid_dims,g->pgrid);
	rank = grid->rank;
    ...
    {
		grid = new Grid(g->grid_dims,g->pgrid,g->grid_comm,g->grid_id, g->ngrids, g->all_grids);
int start=0,end,grank;
	for(int i=0;i<g->grid_id;i++) start+=g->all_grids[i]; end=start+g->all_grids[g->grid_id];
	MPI_Comm_rank(MPI_COMM_WORLD,&grank);
	if(grank<start || grank>=end){return;}
...
// Receive data at instigator
	...
	for(int i=0; i<num_senders; i++) 
	{
		...
		MPI_Probe(senders[i], 1, grid->grid_comm, &status);
...
// Returns number of block copied
	...
	MPI_Isend(&source_num_blocks, 1, MPI_INT, next, 0, grid->grid_comm, &req_num);
	MPI_Recv(&receive_num_blocks, 1, MPI_INT, prev, 0, grid->grid_comm, &recv_st_num);
	...
	MPI_Isend(source_data, source_num_blocks * block_size, 
		  MPI_DOUBLE, next, 1, grid->grid_comm, &req_data);

	MPI_Isend(source_addr, source_num_blocks * dims, 
		  MPI_INT, next, 2, grid->grid_comm, &req_addr);
	...
	MPI_Recv(receive_data, receive_num_blocks * block_size, 
		 MPI_DOUBLE, prev, 1, grid->grid_comm, &recv_st_num);

	MPI_Recv(receive_addr, receive_num_blocks * dims, 
		 MPI_INT, prev, 2, grid->grid_comm, &recv_st_num);
	...
	if(rank != next)
	{
		...
		MPI_Send(data, size, MPI_INT, next, 1, grid->grid_comm);
	    }
	    else 
	    {
		MPI_Status st;
		...
		MPI_Recv(data, size, MPI_INT, prev, 1, grid->grid_comm, &st);
	...
	if(rank != next)
	{
	    if(is_instigator) // If this is the instigator
	    {
		MPI_Send(data, size, MPI_DOUBLE, next, 1, grid->grid_comm);
	    }
	    else 
	    {
		MPI_Status st;
		MPI_Recv(data, size, MPI_DOUBLE, prev, 1, grid->grid_comm, &st);
		if(k % pgrid[A->index_dimension_map[cdim]] != (my_address[A->index_dimension_map[cdim]] + 1) % pgrid[A->index_dimension_map[cdim]])
		{
		    MPI_Send(data, size, MPI_DOUBLE, next, 1, grid->grid_comm);
		}
...
//of the output tensor
	...
	MPI_Comm_split(grid->grid_comm, color, rank, &reduction_comm);
...
// Perform contraction
    ...
    {
		int start=0,end,grank;
	for(int i=0;i<grid->grid_id;i++) start+=grid->all_grids[i]; end=start+grid->all_grids[grid->grid_id];
	MPI_Comm_rank(MPI_COMM_WORLD,&grank);
	if(grank<start || grank>=end){return;}
	int checkpoint =0;
	...
	if(changed)
	{
	    ...
	    Grid* new_grid = new Grid(gdim, npgrid,grid->grid_comm,grid->grid_id, grid->ngrids, grid->all_grids);
 ...
 * same physical dimension*/
	...
	if(DISPLAY_TIME)
	{
	    MPI_Reduce(&timer1, &m_timer1, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&timer2, &m_timer2, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&timer3, &m_timer3, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&timer4, &m_timer4, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&timer5, &m_timer5, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&timer6, &m_timer6, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
    
	    MPI_Reduce(&total_time, &max_total_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&redist_time, &max_redist_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&comm_time, &max_comm_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&instigation_time, &max_instigation_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&forward_receive_time, &max_forward_receive_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&block_size_forward_time, &max_block_size_forward_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(
		&address_forward_time, &max_address_forward_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&data_forward_time, &max_data_forward_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);

	    MPI_Reduce(&comp_time, &max_comp_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&tr_time, &max_tr_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&bm_time, &max_bm_time, 1, MPI_DOUBLE, MPI_MAX, 0, grid->grid_comm);
	    MPI_Reduce(&local_num_dgemm, &total_num_dgemm, 1, MPI_INT, MPI_SUM, 0, grid->grid_comm);
	    print_time_flops();
	}
